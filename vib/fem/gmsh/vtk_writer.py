...
def write_vtu(Verts, Cells, pdata=None, pvdata=None, cdata=None, cvdata=None,
              pname=None, pvname=None, cname=None, cvname=None,
              fname='output.vtu'):
    """
    ...
    for key in Cells:
        if ((type(key) != int) or (key not in range(1, 15))):
            raise ValueError(
                'cell array must have positive integer keys in [1,14]')
    ...
    if pdata is not None:
        ...
        if pdata.shape[0] != Ndof:
            raise ValueError(
                'pdata array should be of length %d (it is now %d)' %
                (Ndof, pdata.shape[0]))
    ...
    if cdata is not None:
        for key in Cells:   # all valid now
            ...
            if cdata[key] is None:
                raise ValueError(
                    'cdata array cannot be empty for key %d' % (key))
    ...
    if cvdata is not None:
        for key in Cells:   # all valid now
            ...
            if cvdata[key] is None:
                raise ValueError(
                    'cvdata array cannot be empty for key %d' % (key))

    Ncells = 0

    cell_ind = []
    # = numpy.zeros((Ncells,1),dtype=uint8) # offsets are zero indexed
    cell_offset = []
    ...
    for key in Cells:
        ...
        cell_offset = numpy.hstack(
            (cell_offset, offset * numpy.ones((sz,), dtype='uint8')))
        cell_type = numpy.hstack(
            (cell_type, key * numpy.ones((sz,), dtype='uint8')))
    ...
    for i in range(0, n_pdata):
        pn = pname[i] if pname is not None else 'pdata %d' % (i)
        pdata_obj.append(doc.createElementNS('VTK', 'DataArray'))
        d = {'type': 'Float32', 'Name': pn,
    ...
    for i in range(0, n_pvdata):
        pn = pvname[i] if pvname is not None else 'pvdata %d' % (i)
    ...
    for i in range(0, n_cdata):
        pn = cname[i] if cname is not None else 'cdata %d' % (i)
    ...
    for i in range(0, n_cvdata):
        pn = cvname[i] if cvname is not None else 'cvdata %d' % (i)
...
def write_basic_mesh(Verts, E2V=None, mesh_type='tri',
    ...
    if mesh_type == 'vertex':
        E2V = {key: numpy.arange(
            0, Verts.shape[0]).reshape((Verts.shape[0], 1))}
    else:
        E2V = {key: E2V}
